...
- Endpoint: `POST /summarize-messages`

The AI Engine summarizes messages that were filtered during the current focus session, allowing the user to quickly review what happened while staying focused.
...
```json
{
  "messages": [
    {
      "id": "slack:summary-001",
      "source": "slack",
      "sender": "teammate",
      "title": null,
      "content": "Demo 文件我已經更新好了。",
      "timestamp": "2026-09-20T00:20:00+08:00",
      "unread": true
    },
    {
      "id": "gmail:summary-002",
      "source": "gmail",
      "sender": "organizer@example.com",
      "title": "活動場地更新",
      "content": "明天活動場地改到工程一館 101 教室。",
  ...
  "context": {
    "mode": "focus",
    "currentTask": "Prepare Hackathon Demo",
    "automationMode": "off",
    "focusStartedAt": "2026-09-19T23:30:00+08:00",
...
#### Request Fields
##### `messages`
A list of messages filtered during the current focus session.

Each message follows the `ExternalMessage` format:

- `id`: Unique identifier of the message.
- `source`: Message source. Currently supports:"gmail" or "slack"
- `sender`: Message sender.
- `title`: Message title. Can be null.
- `content`: Message content.
- `timestamp`: Message timestamp in ISO 8601 format.
...
- `metadata`: Optional additional information.
##### `context`
- `mode`: Current application mode:"idle" / "focus" / "auto"
- `currentTask`: Current task of the user. Can be null.
- `automationMode`: Current automation behavior: "on" / "suggestion" / "off"
- `focusStartedAt`: Start time of the current focus session. Can be null.
- `focusDurationSeconds`: Duration of the focus session in seconds. Can be null.
- `allowedApps`: Optional list of allowed applications.
- `blockedApps`: Optional list of blocked applications.
...
Optional extension data for the summary request.

#### Response Format
```
{
  "messageCount": 2,
  "summary": "專注期間有兩則被暫緩的訊息：隊友已更新 Demo 文件；明天活動場地改至工程一館 101 教室。",
  "metadata": null
}
```
##### Response Fields
- `messageCount`: Number of input messages included in the summary. This value is calculated by the AI Engine from the request and is not generated by the language model.
- `summary`: AI-generated concise summary of the filtered messages in Traditional Chinese.
- `metadata`: Optional additional output information.